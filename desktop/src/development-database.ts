 ...
 * VACUUM INTO takes a consistent SQLite snapshot: copying the main file can
...
export async function seedDevelopmentDatabase(
  ...
  try {
    ...
    source.prepare('VACUUM INTO ?').run(temporaryDatabase);
...
function isolateDevelopmentData(
  ...
  try {
    ...
    database.exec('PRAGMA busy_timeout = 5000; PRAGMA journal_mode = DELETE; BEGIN IMMEDIATE;');
    try {
      ...
      if (tableExists(database, 'password_vault_key_cleanup')) {
        if (copiedFromInstalledApp) {
          ...
          database
            .prepare<Record<string, string>, string[]>('DELETE FROM password_vault_key_cleanup WHERE vault_id NOT LIKE ?')
      ...
      if (tableExists(database, 'password_vault')) {
        const vault = database
          .prepare<Record<string, string>, string[]>('SELECT vault_id, unlock_policy FROM password_vault WHERE singleton = 1')
          .get();
        if (vault) {
          ...
          if (copiedFromInstalledApp || !sourceVaultId.startsWith(DEVELOPMENT_VAULT_PREFIX)) {
            ...
            database
              .prepare<Record<string, string>, string[]>('UPDATE password_vault SET vault_id = ? WHERE singleton = 1')
              .run(nextVaultId);
            remap = {
              sourceVaultId,
...
function tableExists(database: Database, table: string): boolean {
  return !!database
    .prepare<Record<string, string>, string[]>("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ?")
    .get(table);
}
