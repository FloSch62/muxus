...
- **Kitty keyboard protocol** — the progressive-enhancement flag stack (disambiguate, event types, alternate keys, report-all, associated text), so modern TUIs (neovim, helix, fish) get full key fidelity. `TERM=xterm-256color` is the compatibility default; `xterm-kitty` is available for hosts with its terminfo installed.
...
- **Terminal quality of life** — incremental scrollback search (case / whole-word / regex), export the buffer as text or color-preserving HTML, copy-all, clear-scrollback, select-all, per-tab zoom (Ctrl+wheel or keyboard), a configurable right-click (copy/paste convention, always-paste, or context menu), and a confirmation preview before multiline text can execute several shell commands. Tabs rename (double-click), duplicate and take color flags.
- **Live settings** — a sectioned settings dialog (appearance, terminal, behavior, shortcuts) whose changes apply immediately to open terminals: terminal color schemes (Muxus, Dracula, One Dark, Nord, Gruvbox, Catppuccin, Monokai, Solarized), font family/size/line height, cursor, scrollback and clipboard behavior.
