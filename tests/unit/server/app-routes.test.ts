  ...
  it('validates update manifests and reports a newer trusted release', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          version: 'v0.4.0',
          releaseName: 'Muxus 0.4',
          releaseUrl: 'https://github.com/FloSch62/muxus/releases/tag/v0.4.0',
    ...
    expect(fetchMock.mock.calls[0]?.[1]).toMatchObject({
      headers: { Accept: 'application/json', 'User-Agent': 'Muxus/0.3.1' },
    });
  });
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            version: '0.4.0',
            releaseUrl: 'https://attacker.example/FloSch62/muxus/releases/tag/v0.4.0',
          }),
          { status: 200, headers: { 'content-type': 'application/json' } },
        ),
      ),
