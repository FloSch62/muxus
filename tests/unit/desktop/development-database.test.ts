...
} from '../../../desktop/src/development-database.js';
...
function directories(): { installed: string; development: string } {
  temporaryDirectory = mkdtempSync(path.join(os.tmpdir(), 'muxus-desktop-database-'));
...
function openDatabase(directory: string): Database {
  return new Database(path.join(directory, 'muxus.sqlite3'));
}
...
describe('Electrobun development database', () => {
