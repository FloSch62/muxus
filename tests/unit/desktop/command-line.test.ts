...
} from '../../../desktop/src/command-line.js';

describe('desktop command-line launch parsing', () => {
  it('accepts split and equals forms among unrelated Electrobun arguments', () => {
    expect(parseCommandLineLaunch(['muxus', '--host', 'edge-1'])).toEqual({
      kind: 'host',
      name: 'edge-1',
    });
    expect(
      parseCommandLineLaunch(['muxus', '.', '--inspect=9229', '--folder=Production/EU']),
