...
Muxus is a pnpm workspace of four TypeScript/ESM packages plus a test package. Everything
runs on your machine: a Fastify server on loopback, a React client, and an Electron shell
that embeds both.
...
| Package | What it is |
| --- | --- |
| `shared/` | REST DTOs and the zod-validated WebSocket protocol. On `/ws/terminal`, **binary frames are bytes** and **text frames are control messages** — no framing invented on top of either. |
| `server/` | Fastify bound to `127.0.0.1` with a per-run bearer token; a small versioned SQLite database; the line-preserving `ssh_config` parser/resolver/editor; leased `ssh2` transports with ProxyJump and OpenSSH-order authentication; `known_hosts` verification; `node-pty` local shells; Telnet negotiation; cross-platform `serialport` access; SFTP routes and the forward manager; the session-history worker. |
...
so terminals are never unmounted — which is why splitting, closing and moving tabs cannot
disturb a running shell, its scrollback, or its SSH channel.

**Connections are leased.** One SSH transport serves every consumer that asks for the same
host: extra terminals, the SFTP panel, the remote editor, ad-hoc forwards. A saved tunnel
takes its own lease, so it survives the closing of every terminal.

**The config is a document, not a model.** The parser records exactly which lines in which
file each block owns, so an edit rewrites those lines and nothing else — comments,
ordering and unmodelled options survive untouched. Writes are atomic with a `.muxus.bak`.

**Session history is not in the application database.** A dedicated worker writes framed
raw events into rotated zstd segments and batches normalized transcript chunks into a
separate FTS5 database, with its own quota, eviction and recovery. A terminal is never
blocked by the recorder.

**Graphics stream, not buffer.** Kitty APC sequences flow straight from xterm.js 6.1 into
the Image Addon: chunked payloads are parsed incrementally, base64 decoding runs through
...
**Keys are one table.** Every command — panes, tabs, terminal, application — is a keymap
entry with default chords, dispatched before the terminal sees the event. A command that is
not applicable returns "not handled", and the key falls through to the shell.
...
2. The server dials it — hop by hop for a jump chain — verifying host keys and running the
   OpenSSH authentication order, prompting through the socket when a human is needed.
3. A shell channel opens; **binary frames** carry bytes both ways.
...
5. The lease keeps the transport alive for the file browser, the editor and any forwards
   until the last consumer lets go.
